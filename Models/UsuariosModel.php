<?php 
	...
	class UsuariosModel extends Mysql
	{
		...
		public function insertUsuario(string $identificacion, string $nombre, string $apellido, int $telefono, string $email, string $password, int $tipoid, int $status){
			...
			if(empty($request))
			{
				$query_insert  = "INSERT INTO usuarios(identificacion,nombres,apellidos,telefono,correo_usuario,contraseña,rolid,status) 
								  VALUES(?,?,?,?,?,?,?,?)";
		...
		public function selectUsuarios()
		{
			...
			$sql = "SELECT p.idusuario,p.identificacion,p.nombres,p.apellidos,p.telefono,p.correo_usuario,p.status,r.idrol,r.nombrerol 
					FROM usuarios p 
					INNER JOIN rol r
					ON p.rolid = r.idrol
		...
		public function selectUsuario(int $idpersona){
			...
			$sql = "SELECT p.idusuario,p.identificacion,p.nombres,p.apellidos,p.telefono,p.correo_usuario,p.nombrefiscal,p.direccionfiscal,r.idrol,r.nombrerol,p.status, DATE_FORMAT(p.fecha_creada, '%d-%m-%Y') as fechaRegistro 
					FROM usuarios p
					INNER JOIN rol r
					ON p.rolid = r.idrol
		...
		public function updateUsuario(int $idUsuario, string $identificacion, string $nombre, string $apellido, int $telefono, string $email, string $password, int $tipoid, int $status){
			...
			if(empty($request))
			{
				if($this->strPassword  != "")
				{
					$sql = "UPDATE usuarios SET identificacion=?, nombres=?, apellidos=?, telefono=?, correo_usuario=?, contraseña=?, rolid=?, status=? 
					...
					$sql = "UPDATE usuarios SET identificacion=?, nombres=?, apellidos=?, telefono=?, correo_usuario=?, rolid=?, status=? 
							WHERE idusuario = $this->intIdUsuario ";
					$arrData = array($this->strIdentificacion,
	        						$this->strNombre,
	        						$this->strApellido,
		...
		public function updatePerfil(int $idUsuario, string $identificacion, string $nombre, string $apellido, int $telefono, string $password){
			...
			if($this->strPassword != "")
			{
				$sql = "UPDATE usuarios SET identificacion=?, nombres=?, apellidos=?, telefono=?, contraseña=? 
						WHERE idusuario = $this->intIdUsuario ";
				$arrData = array($this->strIdentificacion,
								$this->strNombre,
								$this->strApellido,
		...
		public function updateDataFiscal(int $idUsuario, string $strNomFiscal, string $strDirFiscal){
			...
			$sql = "UPDATE usuarios SET nombrefiscal=?, direccionfiscal=? 
						WHERE idusuario = $this->intIdUsuario ";
			$arrData = array($this->strNomFiscal,
							$this->strDirFiscal);
			$request = $this->update($sql,$arrData);
		    return $request;
		}
